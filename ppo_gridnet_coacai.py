"""
PPO GridNet CoacAI 에이전트 훈련 스크립트

이 스크립트는 GridNet 아키텍처를 사용하여 CoacAI와 대전하는 PPO 에이전트를 훈련합니다.
완전한 액션 마스킹을 사용하여 유효하지 않은 액션을 제거합니다.

주요 특징:
- GridNet 아키텍처 (각 그리드 셀에 대해 액션 예측)
- CoacAI와의 대전
- 완전한 액션 마스킹 (유효하지 않은 액션 제거)
- 16x16 그리드 기반 액션 공간
...
# ==================== MicroRTS 환경 생성 ====================
# GridNet 모드로 MicroRTS 환경 생성 (CoacAI와 대전)
envs = MicroRTSGridModeVecEnv(
    ...
    ai2s=[microrts_ai.coacAI for _ in range(args.num_bot_envs)],  # CoacAI 상대
...
class Agent(nn.Module):
    ...
    CNN 기반 특징 추출기와 액터-크리틱 헤드를 사용하여
    각 그리드 셀에 대해 액션을 예측합니다.
    """
    def __init__(self, mapsize=16*16):
        ...
        self.network = nn.Sequential(
            layer_init(nn.Conv2d(27, 16, kernel_size=3, stride=2)),  # 27채널 입력
            nn.ReLU(),
            layer_init(nn.Conv2d(16, 32, kernel_size=2)),  # 32채널로 확장
            nn.ReLU(),
            nn.Flatten(),  # 1차원으로 평탄화
            layer_init(nn.Linear(32*6*6, 256)),  # 256차원 특징 벡터
            nn.ReLU(),)
        
        # 액터 헤드: 각 그리드 셀에 대한 액션 로짓 예측
        self.actor = layer_init(nn.Linear(256, self.mapsize*envs.action_space.nvec[1:].sum()), std=0.01)
        # 크리틱 헤드: 상태 가치 예측
        self.critic = layer_init(nn.Linear(256, 1), std=1)

    def forward(self, x):
        """관찰을 특징 벡터로 변환 (BHWC -> BCHW)"""
