"""
PPO CoacAI 에이전트 훈련 스크립트

이 스크립트는 CoacAI와 대전하는 PPO 에이전트를 훈련합니다.
전체 액션 마스킹을 사용하는 버전입니다.

주요 특징:
- CoacAI와의 대전
- 전체 액션 마스킹 (모든 액션 파라미터 마스킹)
- 단위 기반 액션 공간
...
# MicroRTS 환경 생성 (CoacAI와 대전)
envs = MicroRTSVecEnv(
    num_envs=args.num_envs,  # 병렬 환경 수
    max_steps=2000,          # 최대 스텝 수
    render_theme=2,          # 렌더링 테마
    ai2s=[microrts_ai.coacAI for _ in range(args.num_envs)],  # 상대 AI (CoacAI)
...
class Agent(nn.Module):
    """
    PPO 에이전트 네트워크
    CNN 기반 특징 추출기와 액터-크리틱 헤드를 포함합니다.
    """
    def __init__(self, frames=4):
        super(Agent, self).__init__()
        # CNN 기반 특징 추출 네트워크
        self.network = nn.Sequential(
            layer_init(nn.Conv2d(27, 16, kernel_size=3, stride=2)),  # 첫 번째 컨볼루션 레이어
            nn.ReLU(),
            layer_init(nn.Conv2d(16, 32, kernel_size=2)),  # 두 번째 컨볼루션 레이어
            nn.ReLU(),
            nn.Flatten(),  # 2D 특징을 1D로 평면화
