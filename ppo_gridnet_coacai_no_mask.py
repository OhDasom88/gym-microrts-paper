"""
PPO GridNet CoacAI No Mask 에이전트 훈련 스크립트

이 스크립트는 GridNet 아키텍처를 사용하여 CoacAI와 대전하는 PPO 에이전트를 훈련합니다.
액션 마스킹을 사용하지 않아 모든 액션이 허용되며, 그리드 기반 액션 선택을 사용합니다.
...
class Agent(nn.Module):
    ...
    def get_action(self, x, action=None, invalid_action_masks=None, envs=None):
        """
        액션 선택 및 로그 확률, 엔트로피 계산 (마스킹 없음)
        
        Args:
            x: 관찰값
            action: 미리 선택된 액션 (None이면 새로 샘플링)
            invalid_action_masks: 유효하지 않은 액션 마스크 (사용되지 않음)
            envs: 환경 객체
        ...
        Returns:
            ...
            logprob: 로그 확률
            entropy: 엔트로피
            invalid_action_masks: 액션 마스크 (모두 1로 설정)
            real_invalid_action_masks: 실제 액션 마스크
        """
