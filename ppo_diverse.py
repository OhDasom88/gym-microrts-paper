"""
PPO Diverse 에이전트 훈련 스크립트

이 스크립트는 다양한 AI와 대전하여 PPO 에이전트를 훈련합니다.
단위 기반 액션 공간을 사용하며, 다양한 상대 AI와의 대전을 통해 강건한 에이전트를 학습합니다.

주요 특징:
- 다양한 AI와의 대전 (CoacAI, RandomBiasedAI, LightRushAI, WorkerRushAI)
- 단위 기반 액션 선택
- 액션 마스킹을 통한 유효하지 않은 액션 제거
- PPO 알고리즘을 사용한 정책 최적화
...
class VecPyTorch(VecEnvWrapper):
    ...
    def reset(self):
        """환경 리셋 및 관찰값을 PyTorch 텐서로 변환"""
        obs = self.venv.reset()
        obs = torch.from_numpy(obs).float().to(self.device)
        return obs

    def step_async(self, actions):
        """비동기 액션 실행 (PyTorch 텐서를 numpy로 변환)"""
        actions = actions.cpu().numpy()
        self.venv.step_async(actions)

    def step_wait(self):
        """스텝 결과 대기 및 PyTorch 텐서로 변환"""
...
# ==================== MicroRTS 환경 생성 ====================
# 다양한 AI와의 대전을 위한 환경 설정
envs = MicroRTSVecEnv(
    num_envs=args.num_envs,                    # 병렬 환경 수
    max_steps=2000,                           # 최대 게임 스텝 수
    render_theme=2,                           # 렌더링 테마
    # 다양한 AI 조합: CoacAI(18개), RandomBiasedAI(2개), LightRushAI(2개), WorkerRushAI(2개)
...
class Agent(nn.Module):
    ...
    def get_action(self, x, action=None, invalid_action_masks=None, envs=None):
        ...
        if action is None:
            ...
            # 2. 액션 타입 및 파라미터 선택 (소스 유닛의 액션 마스크 기반)
...
# `next_obs`와 `next_done`의 사용법은 다음과 동일:
